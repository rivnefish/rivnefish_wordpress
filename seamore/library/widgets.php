...
class LoginWidget extends WP_Widget{
	...
	function widget($args, $instance){
		...
		?>
			<ul class="alignleft">
				<li><a href="<?php bloginfo('wpurl') ?>/wp-admin/"><?php _e('Панель керування', THEME_NS); ?></a></li>
				<?php if ($user_level >= 1): ?>
				<li><a href="<?php bloginfo('wpurl') ?>/wp-admin/post-new.php"><?php _e('Створити допис', THEME_NS); ?></a></li>
				<li><a href="<?php bloginfo('wpurl') ?>/wp-admin/edit-comments.php"><?php _e('Коментарі', THEME_NS); ?></a></li>
				<?php endif; ?>
				<li><a href="<?php echo wp_logout_url() ?>&amp;redirect_to=<?php echo urlencode(theme_get_current_url()); ?>"><?php _e("Log out", THEME_NS); ?></a></li>
			</ul>
	  	<?php else:
			_e('Форма входу', THEME_NS);
			echo $after_title; ?>

			<form action="<?php bloginfo('wpurl') ?>/wp-login.php" method="post">
				<label for="log"><?php _e('Ім\'я', THEME_NS) ?></label><br /><input type="text" name="log" id="log" value="<?php echo esc_attr(stripslashes($user_login), 1) ?>" size="20" /><br />
				<label for="pwd"><?php _e("Пароль", THEME_NS); ?></label><br /><input type="password" name="pwd" id="pwd" size="20" /><br />
				<span class="art-button-wrapper">
					<span class="art-button-l"> </span>
					<span class="art-button-r"> </span>
					<input class="art-button" type="submit" name="submit" value="<?php echo esc_attr(__('Увійти', THEME_NS)); ?>" class="button" />
				</span>	
				<br />
				<label for="rememberme"><input name="rememberme" id="rememberme" type="checkbox" checked="checked" value="forever" /><?php _e('Пам\'ятати мене', THEME_NS); ?></label><br />
				<input type="hidden" name="redirect_to" value="<?php echo theme_get_current_url(); ?>"/>
			</form>
			<ul>
				<?php if (get_option('users_can_register')) { ?><li><a href="<?php bloginfo('wpurl') ?>/wp-register.php"><?php _e("Зареєструватись", THEME_NS); ?></a></li><?php } ?>
				<li><a href="<?php bloginfo('wpurl') ?>/wp-login.php?action=lostpassword"><?php _e("Забули свій пароль?", THEME_NS); ?></a></li>
			</ul>
		<?php endif; ?>
